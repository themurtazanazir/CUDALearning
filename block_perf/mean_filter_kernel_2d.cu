...
using namespace std;
__global__
void mean_filter_kernel_2d(unsigned char *output, unsigned char *input, int width, int height, int radius)
{
...
torch::Tensor mean_filter_2d(torch::Tensor image, int radius)
{
    ...
    mean_filter_kernel_2d<<<blocks_per_grid, threads_per_block, 0, torch::cuda::getCurrentCUDAStream()>>>(
